...
DEFAULT_CONFIG="bluejay/private/gs-google/build.config.slider"
...
if [ "${BUILD_KERNEL}" = "0" ]; then
  ...
  if [ "${EXPERIMENTAL_BUILD}" != "0" ]; then
    ...
    GKI_KERNEL_BUILD_CONFIG=bluejay/common/build.config.gki.aarch64
  else
    GKI_KERNEL_OUT_DIR=android12-5.10
    GKI_KERNEL_BUILD_CONFIG=bluejay/aosp/build.config.gki.aarch64
  fi
fi
...
if [ "${CHECK_DIRTY_AOSP}" != "0" ]; then
  ...
  pushd "${script_path}/../../aosp/" > /dev/null
